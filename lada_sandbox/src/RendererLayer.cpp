...
void RendererLayer::OnAttach(Lada::App::LayerContext *context) {
    ...
    std::shared_ptr<Lada::Render::Material> material = Lada::Render::MaterialBuilder(workingDirectory + "/lada_sandbox/res/shaders/Basic.glsl")
    ...
    std::shared_ptr<Lada::Render::Mesh> squareMesh = Lada::Render::MeshBuilder(positions, 4 * 4 * sizeof(float))
    ...
    m_Square = std::make_shared<Lada::Render::Model>(squareMesh, material);
}
...
void RendererLayer::OnRender(Lada::App::LayerContext *context, const std::shared_ptr<Lada::Render::Renderer>& renderer) {
